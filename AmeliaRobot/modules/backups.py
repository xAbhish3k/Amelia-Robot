...
# from Robot.modules.sql import cust_filters_sql as filtersql
# import Robot.modules.sql.welcome_sql as welcsql
...
def export_data(update, context):
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("AmeliaRobot{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `AmeliaRobot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("AmeliaRobot{}.backup".format(chat_id))  # Cleaning file
