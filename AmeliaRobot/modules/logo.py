...
async def lego(event):
    ...
    try:
        ...
        fillcolor = "black"
        shadowcolor = "blue"
        fnt = glob.glob("./AmeliaRobot/resources/fonts/*")
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ ᴀᴍᴇʟɪᴀ ʀᴏʙᴏᴛ"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @warbotzsupport")


__mod_name__ = "Lᴏɢᴏ​"

__help__ = """
Amelia Robot can create some beautiful and attractive logo for your profile pics.
